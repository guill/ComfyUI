...
class TypeTracker:
    ...
    def __init__(self):
        ...
        self.builtin_types = {
            ...
            'Sequence', 'cast', 'NamedTuple', 'str', 'int', 'float', 'bool',
            'None', 'bytes', 'object', 'type', 'dict', 'list', 'tuple', 'set'
        }
        self.already_imported = set()  # Track types already imported to avoid duplicates
    ...
    def track_type(self, annotation):
        ...
        if type_name and (type_name in self.builtin_types or type_name in self.already_imported):
            return
...
class AsyncToSyncConverter:
    ...
    def create_sync_class(cls, async_class: Type, thread_pool_size=10) -> Type:
        ...
        for name, method in inspect.getmembers(async_class, predicate=inspect.isfunction):
        ...
        for name, prop in inspect.getmembers(async_class, lambda x: isinstance(x, property)):
    ...
    @classmethod
    def _format_type_annotation(cls, annotation, type_tracker: Optional[TypeTracker] = None) -> str:
        ...
        try:
            ...
            if origin is not None:
                ...
                if args:
                    formatted_args = [cls._format_type_annotation(arg, type_tracker) for arg in args]
    ...
    @classmethod
    def _format_method_parameters(cls, sig: inspect.Signature, skip_self: bool = True, type_tracker: Optional[TypeTracker] = None) -> str:
    ...
    @classmethod
    def _generate_method_signature(cls, method_name: str, method, is_async: bool = False, type_tracker: Optional[TypeTracker] = None) -> str:
    ...
    @classmethod
    def _generate_imports(cls, async_class: Type, type_tracker: TypeTracker) -> list[str]:
        """Generate import statements for the stub file."""
        imports = []

        # Add standard typing imports
        imports.append("from typing import Any, Dict, List, Optional, Tuple, Union, Set, Sequence, cast, NamedTuple")
        ...
        if async_class.__module__ != "builtins":
            ...
            if additional_types:
                ...
                imports.append(f"from {async_class.__module__} import {async_class.__name__}")

        # Add imports for all discovered types
        # Pass the main module name to avoid duplicate imports
        imports.extend(type_tracker.get_imports(main_module_name=async_class.__module__))
    ...
    def _get_class_attributes(cls, async_class: Type) -> list[tuple[str, Type]]:
        ...
        for name, attr in sorted(inspect.getmembers(async_class)):
            ...
            elif hasattr(async_class, "__annotations__") and name in async_class.__annotations__:
    ...
    @classmethod
    def _generate_inner_class_stub(cls, name: str, attr: Type, indent: str = "    ", type_tracker: Optional[TypeTracker] = None) -> list[str]:
        ...
        if hasattr(attr, '__doc__') and attr.__doc__:
            stub_lines.extend(cls._format_docstring_for_stub(attr.__doc__, f"{indent}    "))
        ...
        if hasattr(attr, '__init__'):
            try:
                ...
                # Format parameters
                params_str = cls._format_method_parameters(init_sig, type_tracker=type_tracker)
                # Add __init__ docstring if available (before the method)
                if hasattr(init_method, '__doc__') and init_method.__doc__:
                    stub_lines.extend(cls._format_docstring_for_stub(init_method.__doc__, f"{indent}    "))
                stub_lines.append(f"{indent}    def __init__({params_str}) -> None: ...")
            except (ValueError, TypeError):
                stub_lines.append(f"{indent}    def __init__(self, *args, **kwargs) -> None: ...")

        # Add methods to the inner class
        has_methods = False
        for method_name, method in sorted(inspect.getmembers(attr, predicate=inspect.isfunction)):
            ...
            try:
                ...
                if method.__doc__:
                    stub_lines.extend(cls._format_docstring_for_stub(method.__doc__, f"{indent}    "))

                method_sig = cls._generate_method_signature(method_name, method, is_async=True, type_tracker=type_tracker)
                stub_lines.append(f"{indent}    {method_sig}")
            except (ValueError, TypeError):
                stub_lines.append(f"{indent}    def {method_name}(self, *args, **kwargs): ...")
    ...
    @classmethod
    def _format_docstring_for_stub(cls, docstring: str, indent: str = "    ") -> list[str]:
    ...
    def generate_stub_file(cls, async_class: Type, sync_class: Type) -> None:
        ...
        try:
            ...
            # Create stub file path in the same directory as the module
            stub_dir = os.path.dirname(module_path)
            ...
            if async_class.__doc__:
                stub_content.extend(cls._format_docstring_for_stub(async_class.__doc__, "    "))
            ...
            try:
                ...
                # Format parameters
                params_str = cls._format_method_parameters(init_signature, type_tracker=type_tracker)
                # Add __init__ docstring if available (before the method)
                if hasattr(init_method, '__doc__') and init_method.__doc__:
                    stub_content.extend(cls._format_docstring_for_stub(init_method.__doc__, "    "))
                stub_content.append(f"    def __init__({params_str}) -> None: ...")
            except (ValueError, TypeError):
                stub_content.append("    def __init__(self, *args, **kwargs) -> None: ...")
            ...
            for name, attr in class_attributes:
                inner_class_stub = cls._generate_inner_class_stub(name, attr, type_tracker=type_tracker)
            ...
            for name, method in sorted(inspect.getmembers(async_class, predicate=inspect.isfunction)):
                ...
                try:
                    method_sig = cls._generate_method_signature(name, method, is_async=True, type_tracker=type_tracker)

                    # Add docstring if available (before the method signature for proper formatting)
                    if method.__doc__:
                        stub_content.extend(cls._format_docstring_for_stub(method.__doc__, "    "))
            ...
            # Add properties
            for name, prop in sorted(inspect.getmembers(async_class, lambda x: isinstance(x, property))):
                stub_content.append("    @property")
                stub_content.append(f"    def {name}(self) -> Any: ...")
                if prop.fset:
                    stub_content.append(f"    @{name}.setter")
                    stub_content.append(f"    def {name}(self, value: Any) -> None: ...")
            ...
            stub_content[imports_placeholder_index:imports_placeholder_index+1] = unique_imports
